// Code created by https://github.com/jonerrr/discord-bitfield-calculator MIT LICENSE
...
/**
 * Class for PermissionManager
 */
export default class PermissionManager {
  /**
   * Permissions BitField to String
   * @param {string} Bitfield
   * @returns {string[]}
   */
  genPermissions = (Bitfield: string) => {
    const currentPermissions: string[] = [];
    const permBitfield = Number(Bitfield);
  ...
  /**
   * Permissions String to BitField
   * @param {string} permission
   * @returns {string[]}
   */
  genBitfield = (permission: string) => {
